...
(function(window, undefined) {
    ...
    clone: function(flag) {
      ...
      if(flag) {
        ...
        this.each(function(key, ele) {
          var temp = ele.cloneNode(true); // including the children node
          // traverse the ele eventCaches
          rickQuery.each(ele.eventsCache, function(name, arr) {
            // travers the array by event name
            rickQuery.each(arr, function(index, method) {
              // add the event to the cloned ele
              $$(temp).on(name, method);
            });
          });
          res.push(temp);
  ...
  // define some event methods on prototype
  rickQuery.prototype.extend({
    
    // define on event method
    on: function(name, fn) {
      // traverse all element
      this.each(function(key, ele) {
        // confirm the current ele reserve the all the event object
        if(!ele.eventsCache) {
          ele.eventsCache = {}
        }
        // confirm the event object reserve the same type array
        if(!ele.eventsCache[name]) {
          ele.eventsCache[name] = [];
          // add callback to array
          ele.eventsCache[name].push(fn);
          // 添加对应类型的事件 add event on the type
          rickQuery.addEvent(ele, name, function() {
            rickQuery.each(ele.eventsCache[name], function(k, method) {
              method();
            })
          });
    ...
    // off method to remove the event
    off: function(name, fn) {
      // confirm it has no argument
      if(0 === arguments.length) {
        this.each(function(key, ele) {
          ele.eventsCache = {};
        })
      ...
      // 
        ...
        this.each(function(key, ele) {
          rickQuery.each(ele.eventsCache[name], function(index, method) {
            // 判断遍历的方法和传入的方法是否相同
            // confirm the traversed method equal to the argument methods
            if(method === fn) {
  ...
  rickQuery.extend({
    ...
    // define compitable event method !!!!!
